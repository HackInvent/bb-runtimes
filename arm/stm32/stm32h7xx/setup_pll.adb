...
--        PLLM_Value  : constant Integer  := 4;
      ...
      PLLN_Value  : constant Integer := 400;
...
--        PLLM        : constant UInt6 := UInt6 (4);
--        PLLN        : constant UInt9 := UInt9 (400);
--        PLLP        : constant UInt2 := UInt2 (2);
--        PLLQ        : constant UInt4 := UInt4 (4);
...
--        RCC_Periph.APB1RSTR.PWRRST := 0;
      ...
      if Activate_PLL then
         ...
         --  Set the source
         RCC_Periph.PLLCKSELR := (others => <>);
         RCC_Periph.PLLCKSELR.PLLSRC := (if HSE_Enabled
                         then PLL_Source'Enum_Rep (PLL_SRC_HSE)
                         else PLL_Source'Enum_Rep (PLL_SRC_HSI));

         --  Set the prescalers to default value
         RCC_Periph.PLLCKSELR.DIVM1 := 16#20#;
         RCC_Periph.PLLCKSELR.DIVM2 := 16#20#;
         RCC_Periph.PLLCKSELR.DIVM3 := 16#20#;
...
--        RCC_Periph.D1CFGR.HPRE := To_AHB (AHB_PRE);
--        RCC_Periph.D1CFGR.D1PPRE := To_APB (APB1_PRE);
--
      RCC_Periph.CFGR.SW := SW_Value;
--        RCC_Periph.CFGR.RTCPRE := 16#0#;
--        RCC_Periph.CFGR.MCO1SEL :=
--        MC01_Clock_Selection'Enum_Rep (MC01SEL_HSI);
--        RCC_Periph.CFGR.MCO2SEL :=
--          MC02_Clock_Selection'Enum_Rep (MC02SEL_SYSCLK);
--        RCC_Periph.CFGR.MCO1PRE := MC0x_Prescaler'Enum_Rep (MC0xPRE_DIV1);
--        RCC_Periph.CFGR.MCO2PRE := MC0x_Prescaler'Enum_Rep (MC0xPRE_DIV5);
--          (SW      => SW_Value,
--           HPRE    => To_AHB (AHB_PRE),
--           PPRE    => (As_Array => True,
